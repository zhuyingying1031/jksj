package com.ady01.demo4.jksj.column;

import com.ady01.demo4.comm.ICollector;
import com.ady01.demo4.comm.WebMagicUtil;
import com.ady01.demo4.jksj.util.CollectorUtil;
    ...
    static class CollectorPageProcessor implements PageProcessor {
        ...
        public void collect() {
            ...
            }
            {
                ...
                request.getHeaders().putAll(CollectorUtil.headerMap());
                request.setMethod("POST");
                String json = String.format("{\"cid\":\"%s\",\"size\":%s,\"prev\":0,\"order\":\"newest\",\"sample\":true}", this.request.getCid(), this.request.getSize());
                request.setRequestBody(HttpRequestBody.json(json, "UTF-8"));
                spider.addRequest(request);
            }
                    ...
                    if (list != null && list.size() >= 1) {
                        ...
                        for (Object o : list) {
                            if (o instanceof JSONObject) {
                                ...
                                articleList.add(ColumnCollectorResponse.Article.builder().id(id).article_title(article_title).build());
                            }
                        }
                    }
                }
